...
class TestDynamoDB():
    ...
    def test_get_flow_timerange_with_first_and_last(self, mock_segments_table):
        ...
        first_segement_result = {
            'Items': [
                {
                    'timerange': first.to_sec_nsec_range()
                }
            ]
        }
        ...
        last_segment_result = {
            'Items': [
                {
                    'timerange': last.to_sec_nsec_range()
                }
            ]
        }

        mock_segments_table.query.side_effect = [
            first_segement_result, last_segment_result]
    ...
    @pytest.mark.parametrize("boundary, inclusivity, conditionType", [
        ([dynamodb.TimeRangeBoundary.START, MS_INCLUDE_START,
         boto3.dynamodb.conditions.LessThan]),
        ([dynamodb.TimeRangeBoundary.START, MS_INCLUDE_END,
         boto3.dynamodb.conditions.LessThanEquals]),
        ([dynamodb.TimeRangeBoundary.END, MS_INCLUDE_START,
         boto3.dynamodb.conditions.GreaterThanEquals]),
        ([dynamodb.TimeRangeBoundary.END, MS_INCLUDE_END,
    ...
    def test_get_timerange_expression(self, boundary, inclusivity, conditionType):
        ...
        result = dynamodb.get_timerange_expression(
            Key,
            boundary,
            time_range
        )
        ...
        expected_value = getattr(
            time_range, other_boundary.value).to_nanosec()
        # Form the expected result. We LessThan here regardless of inputs, but it doesn't impact the test
        expected_condition = Key(
            f'timerange_{boundary.value}').lt(expected_value)

        _, expected_names, expected_values = parse_dynamo_expression(
            expected_condition)
    ...
    def test_get_key_and_args(self):
        ...
        assert result["KeyConditionExpression"].get_expression() == Key(
            "flow_id").eq(flow_id).get_expression()
    ...
    def test_get_key_and_args_object_id_filter(self):
        ...
        assert isinstance(result["KeyConditionExpression"],
                          boto3.dynamodb.conditions.And)
        assert result["IndexName"] == "object-id-index"
        assert set(['flow_id', 'object_id']) == set(
            sorted(actual_names.values()))
    ...
    def test_delete_flow_segments_empty_result(self, merge_delete_request, mock_segments_table, time_range_one_day):
    ...
    @pytest.mark.parametrize("remaining_time_variance", [
    ...
    @patch('dynamodb.merge_delete_request')
    def test_delete_flow_segments_timeout_handling(self, mock_merge_delete_request, mock_delete_segment_items, mock_segments_table, time_range_one_day, remaining_time_variance):
        ...
        mock_segments_table.query.side_effect = [
            {
                "Items": [{"flow_id": "1", "timerange": time_range_one_day.to_sec_nsec_range()}],
    ...
    @patch('dynamodb.segments_table')
    @patch('dynamodb.merge_delete_request')
    def test_delete_flow_segments_error_handling(self, mock_merge_delete_request, mock_segments_table, mock_delete_segment_items, time_range_one_day):
